...
const formatMessageTime = (timestamp) => {
  ...
  if (isToday(messageDate)) {
    ...
    return `${format(messageDate, "dd/MM/yyyy")} ${format(
      messageDate,
      "h:mm a"
    )}`;
  }
...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    if (currentChatData?.chat?.messages) {
      ...
      console.log(
        "📥 Found messages in currentChatData.chat.messages:",
        apiMessages.length
      );
    } else if (currentChatData?.messages) {
      apiMessages = currentChatData.messages;
      console.log(
        "📥 Found messages in currentChatData.messages:",
        apiMessages.length
      );
    }
    ...
    if (apiMessages) {
      ...
      console.log(
        "📥 shouldAcceptApiUpdatesRef:",
        shouldAcceptApiUpdatesRef.current
      );
      ...
      if (
        ...
        console.log(
          "📥 ⏸️ Blocking API updates (optimistic update in progress)"
        );
      }
    } else {
    ...
    (messageData) => {
      ...
      try {
        const audio = new Audio(
          "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+Dyu2keBRKF0fPNeSsFJXfH8N2QQAoUXrTp66hVFApGn+Dyu2keBR"
        );
        audio.volume = 0.3;
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (data) => {
      ...
      const isFromCurrentUser =
        data.sender?._id === user?._id || data.senderId === user?._id;
      ...
      if (data.productId === selectedChatId) {
        console.log(
          "🚀 Adding socket message to local state for immediate display"
        );
      ...
      dispatch(
        chatApi.util.invalidateTags([
          { type: "Chat", id: `auction-${data.productId}` },
        ])
      );
      ...
      if (!isFromCurrentUser) {
        ...
        if (messageProductId !== selectedChatId || showChatList) {
          console.log(
            "📊 Incrementing unread count for product:",
            messageProductId
          );
          ...
          console.log(
            "📖 Not incrementing unread count - user is viewing this chat"
          );
        }
      } else {
        console.log(
          "📖 Not processing notification - message is from current user"
        );
      }
    };
    const handleChatNotification = (data) => {
      ...
      if (data.userId === user?._id && data.senderId !== user?._id) {
        ...
        console.log(
          ...
          messageProductId !== selectedChatId || showChatList
        );

        if (messageProductId !== selectedChatId || showChatList) {
          console.log(
            "📊 Incrementing unread count for notification:",
            messageProductId
          );
          setUnreadCounts((prev) => {
            const newCounts = {
        ...
        console.log(
          "🚫 Ignoring notification - not for current user or from current user"
        );
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 shadow-lg border-b border-gray-200 px-4 py-4 relative z-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              {/* Back arrow for individual chat view */}
                  ...
                  {showChatList
                    ? (() => {
                        try {
                          ...
                          return `${chatList.length} conversation${
                            chatList.length !== 1 ? "s" : ""
                          }${
                          ...
                          console.error(
                            "Error calculating total unread:",
                            error
                          );
                          return `${chatList.length} conversation${
                            chatList.length !== 1 ? "s" : ""
                    ...
                    ? (() => {
                        ...
                        const productTitle =
                          currentChatData?.chat?.product?.title ||
                          "Product Chat";
                        return `${productTitle}${
                          currentUnread > 0 ? ` • ${currentUnread} unread` : ""
                        }`;
                      })()
        ...
        <div className="flex-1 flex overflow-hidden">
          {showChatList ? (
            ...
            <div className="w-full flex flex-col">
              ...
              {/* Chat List */}
              <div
                className="flex-1 overflow-y-auto bg-white scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-400"
                ...
                {isLoadingAuctionChats ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-gray-500">
                    ...
                    <p className="text-sm text-center">
                      Start bidding on auctions to begin chatting with sellers!
                    </p>
                  </div>
                ) : (
...
const ChatView = ({
  ...
  if (localMessages && localMessages.length > 0) {
    ...
    console.log(
      "🎬 Using currentChatData.messages:",
      messages.length,
      "messages"
    );
  } else {
  ...
  console.log(
    "🎬 Chat data structure - localMessages:",
    localMessages?.length || 0
  );
  console.log(
    "🎬 Chat data structure - chatMessages:",
    chat?.messages?.length || 0
  );
  console.log(
    "🎬 Chat data structure - currentChatDataMessages:",
    currentChatData?.messages?.length || 0
  );
  console.log("🎬 Chat data structure - hasChat:", !!chat);
  console.log(
    "🎬 Chat data structure - hasCurrentChatData:",
    !!currentChatData
  );

  const otherUser =
    currentUser?._id === chat?.seller?._id ? chat?.winner : chat?.seller;
          ...
          <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center space-x-3">
            <button
              onClick={(e) => {
                ...
                const productId =
                  chat.product?._id || currentChatData?.product?._id;
                if (productId) {
                  navigate(`/auction/${productId}`);
                } else {
                  // Fallback to previous page if no product ID
                  navigate(-1);
                }
              }}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors flex items-center justify-center"
              title="Go to Auction Details"
            >
              <ArrowLeft className="h-5 w-5 text-gray-600" />
            </button>
            
            <div 
              className="h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs flex-shrink-0 cursor-pointer hover:opacity-80 transition-opacity"
              onClick={() => {
                // Navigate to auction detail page when clicking on avatar
                const productId =
                  chat.product?._id || currentChatData?.product?._id;
                if (productId) {
                  navigate(`/auction/${productId}`);
            ...
            >
              {otherUser?.profilePicture ? (
                ...
                getInitials(otherUser?.fullName || "Unknown User")
              )}
            </div>
            ...
            <div 
              ...
              onClick={() => {
                // Navigate to auction detail page when clicking on user info
                const productId =
                  chat.product?._id || currentChatData?.product?._id;
                if (productId) {
                  navigate(`/auction/${productId}`);
